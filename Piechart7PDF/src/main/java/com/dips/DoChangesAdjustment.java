...
public class DoChangesAdjustment extends BarRenderer {
	...
	public DoChangesAdjustment(final Paint[] colors) {
		this.colors = colors;
	}
	...
	public static void main(String[] args) throws IOException {
		writeChartToPDF(50, 40, "barchartChanges.pdf");
	}
	...
	public static void writeChartToPDF(int width, int height, String fileName) throws FileNotFoundException {
		...
		try {
			...
			java.util.List<JFreeChart> barChartList = chartList.stream()
					.filter(chart -> chart.getTitle().getText().toLowerCase().equals("pie chart")
							|| chart.getTitle().getText().toLowerCase().equals("bar chart"))
					.collect(Collectors.toList());
//			barChartList.stream().sorted(Comparator)

			java.util.List<JFreeChart> otherChartList = chartList.stream()
					.filter(chart -> chart.getTitle().getText().toLowerCase().equals("stacked barchart")
							|| chart.getTitle().getText().toLowerCase().equals("timeseries"))
					.collect(Collectors.toList());

			for (JFreeChart chartList2 : barChartList) {
				System.out.println(chartList2.getTitle().getText().toLowerCase());
			}
			System.out.println("****************");
			for (JFreeChart chartList3 : otherChartList) {
				System.out.println(chartList3.getTitle().getText().toLowerCase());
			}

			for (JFreeChart chart : barChartList) {

				BufferedImage image1 = chart.createBufferedImage(500, 400);
		...
		//	doc.add(new AreaBreak(AreaBreakType.LAST_PAGE));
			for (JFreeChart chart : otherChartList) {
...
//		dataSet.setValue(6070, "Population", "2650 AD");
		...
		final CategoryItemRenderer renderer = new DoChangesAdjustment(
